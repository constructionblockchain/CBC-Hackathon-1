...
class InspectAndRejectCommandTests {
    ...
    private val rejectedJobState = completedJobState.copy(status = JobStatus.REJECTED)

    @Test
    fun `InspectAndReject command should complete successfully`() {
        ledgerServices.ledger {
            transaction {
                command(participants, JobContract.Commands.InspectAndReject())
                input(JobContract.ID, completedJobState)
                output(JobContract.ID, rejectedJobState)
                verifies()
            }
        }
    }
    ...
    fun `The input status must be set as COMPLETED`() {
        ledgerServices.ledger {
            transaction {
                command(participants, JobContract.Commands.InspectAndReject())
                input(JobContract.ID, rejectedJobState)
                output(JobContract.ID, rejectedJobState)
    ...
    @Test
    fun `The output status should be set as REJECTED`() {
        ledgerServices.ledger {
            transaction {
                command(participants, JobContract.Commands.InspectAndReject())
                input(JobContract.ID, completedJobState)
                output(JobContract.ID, completedJobState)
                failsWith("the output status should be set as rejected")
            }
        }
    }
    ...
    fun `Only the status should change`() {
        ledgerServices.ledger {
            transaction {
                command(participants, JobContract.Commands.InspectAndReject())
                input(JobContract.ID, completedJobState)
                output(JobContract.ID, rejectedJobState.copy(developer = thirdParty.party))
    ...
    fun `The developer should be a signer of the transaction`() {
        ledgerServices.ledger {
            transaction {
                command(listOf(contractor.publicKey), JobContract.Commands.InspectAndReject())
                input(JobContract.ID, completedJobState)
                output(JobContract.ID, rejectedJobState)
